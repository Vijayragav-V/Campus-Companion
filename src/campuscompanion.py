...
from .utils.schoology_api import SchoologyAPI
from .utils.task_manager import TaskManager
from .utils.notebook_manager import NotebookManager
from .utils.buttons_manager import ButtonManager
import textwrap
...
class CampusCompanion(customtkinter.CTk):
    def __init__(self):
        ...
        self.sections = SchoologyAPI.get_sections()
        self.grades = SchoologyAPI.get_grades(self.sections)
        self.task_counter = 0
    ...
    def add_task(self):
        ...
        TaskManager.save_task(task_text)
        self.create_task_widgets(task_text)
    ...
    def remove_task(self, task_text, check_box, delete_button):
        ...
        TaskManager.remove_task(task_text)

    def save_notebook(self):
        notebook_text = self.textbox.get("1.0", "end-1c")
        NotebookManager.save_notebook(notebook_text)

    def delete_notebook(self):
        NotebookManager.delete_notebook(self.textbox)

    def upload_notebook(self):
        NotebookManager.upload_notebook(self.textbox)

    def insert_bullet_point(self):
        ButtonManager.insert_bullet_point(self.textbox)

    def change_color(self, color):
        ButtonManager.change_color(self.textbox, color)